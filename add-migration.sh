...
psqldef --enable-drop-table $current_schema < ./schema.sql | grep -v _sqlx_migrations | tail -n +2 > $up_file
psqldef --enable-drop-table ./schema.sql < $current_schema | sed '/_sqlx_migrations/,/);/d' | tail -n +2 > $down_file
